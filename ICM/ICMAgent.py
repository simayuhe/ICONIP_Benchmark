...
from ICM.policies.cnn_gru_policy_dynamics import CnnGruPolicy
# from RND.policies.cnn_policy_param_matched import CnnPolicy
NO_STATES = ['NO_STATES']
...
class InteractionState(object):
    ...
    def __init__(self, ob_space, ac_space, nsteps, gamma, venvs, stochpol, comm):
        ...
        self.buf_obs_next = { k: np.zeros(
                            [nenvs, nsteps] + stochpol.ph_ob[k].shape.as_list()[2:],
                            dtype=stochpol.ph_ob_dtypes[k])
                        for k in stochpol.ph_ob_keys }# icm 
...
#                            "maxfeat", "gradnorm"]
        self._losses = [loss, pg_loss, vf_loss, entropy, clipfrac, approxkl, maxkl, self.stochpol.aux_loss, global_grad_norm]
        self.loss_names = ['tot', 'pg', 'vf', 'ent', 'clipfrac', 'approxkl', 'maxkl', "auxloss", "gradnorm"]
        self.I = None
...
#             logger.info(" "*6 + fmt_row(13, self.loss_names))
        ...
        while epoch < self.nepochs:
            ...
            fd[self.stochpol.ph_ob_next[None]] = np.concatenate([self.I.buf_obs_next[None][mbenvinds], self.I.buf_ob_last[None][mbenvinds, None]], 1)#icm
...
#             print("after call , acts",acs)
            ...
            if t > 0:
                ...
                for k in self.stochpol.ph_ob_keys:# icm  从第二步开始加
                    self.I.buf_obs_next[k][sli, t] = dict_obs[k]
...
#             self.I.buf_rews_int[:] = tf.get_default_session().run(self.stochpol.int_rew, fd)
            ...
            #Calcuate the intrinsic rewards for the rollout. # icm
            fd = {}
            fd[self.stochpol.ph_ob[None]] = np.concatenate([self.I.buf_obs[None], self.I.buf_ob_last[None][:,None]], 1)# 试试把ob_last去掉能跑通吗
            fd[self.stochpol.ph_ob_next[None]] =np.concatenate([self.I.buf_obs_next[None], self.I.buf_ob_last[None][:,None]], 1)
            fd.update({self.stochpol.ph_mean: self.stochpol.ob_rms.mean,
                       self.stochpol.ph_std: self.stochpol.ob_rms.var ** 0.5})
            fd[self.stochpol.ph_ac] = self.I.buf_acs
            self.I.buf_rews_int[:] = tf.get_default_session().run(self.stochpol.int_rew, fd)
