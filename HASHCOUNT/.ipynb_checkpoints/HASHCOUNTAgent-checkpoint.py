...
from RND.utils import explained_variance
# from console_util import fmt_row
from RND.mpi_util import MpiAdamOptimizer, RunningMeanStd, sync_from_root
from HASHCOUNT.policies.cnn_gru_policy_dynamics import CnnGruPolicy
from HASHCOUNT.policies.cnn_policy_param_matched import CnnPolicy
...
class HASHCOUNTAgent():
    def __init__(self,env,args):
        ...
        self.use_news = args.use_news
        self.update_ob_stats_every_step = args.update_ob_stats_every_step
        self.abs_scope = (tf.get_variable_scope().name + '/' + scope).lstrip('/')
        self.testing = args.testing
        self.comm_log = MPI.COMM_SELF
        self.collections_counter=collections.Counter()#HASH
...
#         print("stochpol_fn^^^^^^^^^^^^^^^^^^^",stochpol_fn)
        ...
        with tf.variable_scope(scope):
            ...
            self.nminibatches = args.nminibatches
...
#             print("self.best_ret",self.best_ret,"eprews",eprews)
    ...
    def step(self):
        ...
        for l in range(self.I.nlump):
            ...
            for info in infos:
                ...
                if maybeepinfo: 
                    epinfos.append(info)#重新组合一下传回去
                    self.I.statlists['eprew'].append(info['r'])
                    if self.local_best_ret is None:
                        self.local_best_ret = maybeepinfo
                    elif maybeepinfo > self.local_best_ret:
                        self.local_best_ret = maybeepinfo
...
#                 _, self.I.buf_vpred_int_last[sli], self.I.buf_vpred_ext_last[sli], _, _, _ = self.stochpol.call(dict_nextobs, nextnews, self.I.mem_state[memsli], update_obs_stats=False)
                _, self.I.buf_vpred_int_last[sli], self.I.buf_vpred_ext_last[sli], nlps, _, _ = self.stochpol.call(dict_nextobs, nextnews, self.I.mem_state[memsli], update_obs_stats=False)#HASH
            ...
            # HASH
            o_origin = tf.get_default_session().run(self.stochpol.Original_obs, fd)
            o_decoded = tf.get_default_session().run(self.stochpol.Decoded_obs, fd)
            KL_L = tf.get_default_session().run(self.stochpol.KL_LOSS, fd)
            int_rew_feature= tf.get_default_session().run(self.stochpol.int_rew, fd)
            int_rew_buf=self.hasecount_ae_bounus(int_rew_feature)
            self.I.buf_rews_int[:] = np.reshape(int_rew_buf,(32,128)) #HASH
            ...
            update_info = {}
        return epinfos,self.I.step_count
...
#         return {'update' : update_info}
    def hasecount_ae_bounus(self,x):
        batch_size=4096
        #label_size=x.get_shape()[1]
        int_rews=np.zeros((4096,1))
        x=np.sign((np.sign(x-0.5)+1)/2)#变成0/1编码
#         print("shape",x.shape)
        self.collections_counter.update(map(tuple,x))#储存所有的编解码
        len_counter = len(self.collections_counter)
        max_len_counter =500000
        if len_counter >max_len_counter:
#             print("wwwwwwwwwwwwwwwwwwwwwwwwwwwwww",len_counter)
            list_counter = list(self.collections_counter)
            for ii in range(len_counter-max_len_counter):
                del self.collections_counter[list_counter[ii]]
        for i in range(batch_size):
            int_rews[i,0]=0.02/pow(self.collections_counter[tuple(x[i,:])],0.5)

        int_rews=int_rews.reshape(32,128)
        return int_rews   
