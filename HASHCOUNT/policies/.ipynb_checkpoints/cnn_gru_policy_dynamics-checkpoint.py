...
from RND.stochastic_policy import StochasticPolicy
...
class CnnGruPolicy(StochasticPolicy):
    def __init__(self, scope, ob_space, ac_space,
        ...
        self.sparsityParam=0.5 #用来构造hashcount-ae-rew 的KL lOSS
        ...
        self.generative_net = tf.keras.Sequential(
            [tf.keras.layers.InputLayer(input_shape=(None,512)),
        ...
        # 初始是 （1，1，1024）
        # filter=128, kernel=5, stride=2, out_size = (1  +（5 - 1）× 2 -5 ) / 1 + 1 =5  
        # (5,5,128)
        # filter=64, kernel=5, stride=2, output_size = (5  + （5 - 1）×（2 - 1） +（5 - 1）× 2 - 5 ) / 1 + 1 = 13
        #(13,13,64)
        #filter=32, kernel=9, stride=2, output_size = (13  + （13 - 1）×（2 - 1） +（9- 1）× 2 - 9 ) / 1 + 1 = 33
        #(30,30,32)
        #filter=1, kernel=20, stride=2, output_size = (33  + （33 - 1）×（2 - 1） +（k - 1）× 2 - k ) / 1 + 1 = 84 得 k=20
        #(84,84,1)        
        ...
        if dynamics_bonus:
            ...
            self.define_hashcount_ae_rew(convfeat=convfeat, rep_size=rep_size, enlargement=enlargement)
#             self.define_self_prediction_rew(convfeat=convfeat, rep_size=rep_size, enlargement=enlargement)
    ...
    def define_hashcount_ae_rew(self, convfeat, rep_size, enlargement):
        #ExplorationDRL.
        #HashCount.
        print('hashcount-ae')
...
#                 logger.info("CnnTarget: using '%s' shape %s as image input" % (ph.name, str(ph.shape)))
                xr = ph[:,1:] #(?, ?, 84, 84, 4) # 这里的T是 什么含义
                xr = tf.cast(xr, tf.float32)
                xr = tf.reshape(xr, (-1, *ph.shape.as_list()[-3:]))[:, :, :, -1:]# (?, 84, 84, 1) # 通道数变成1 ？？？
                xr = tf.clip_by_value((xr - self.ph_mean) / self.ph_std, -5.0, 5.0) # (?, 84, 84, 1) 
                self.Original_obs= to2d(xr) #(?, 7056)
                xr = tf.nn.leaky_relu(conv(xr, 'c1r', nf=convfeat * 1, rf=8, stride=4, init_scale=np.sqrt(2)))#(?, 20, 20, 32) # 输入是 (1024,84,84,1) ，filter= 8*8 ,pad=0, stride = [1,4,4,1],N=(W-F+2P)/S+1, (84-8+2*0)/4 +1 = 20
                xr = tf.nn.leaky_relu(conv(xr, 'c2r', nf=convfeat * 2 * 1, rf=4, stride=2, init_scale=np.sqrt(2)))#(?, 9, 9, 64) # (20-4+2*0)/2 +1 = 9
                xr = tf.nn.leaky_relu(conv(xr, 'c3r', nf=convfeat * 2 * 1, rf=3, stride=1, init_scale=np.sqrt(2)))#(?, 7, 7, 64) # (9-3+2*0)1 +1 = 7
                rgbr = to2d(xr)# (?, 3136)
                code_b = fc(rgbr ,'fc1r', nh=rep_size, init_scale=np.sqrt(2)) #b (?, 512)
                #                 rho_approx=tf.reduce_mean(code_b,-1)
                rho_approx=tf.reduce_mean(rgbr,-1) # 为啥用code_b 不行呢？？？
                xr_hat=self.generative_net(code_b)#(?, 84, 84, 1)
                self.Decoded_obs=to2d(xr_hat) #(?, 7056)



        self.KL_LOSS = tf.reduce_mean(self.sparsityParam*tf.math.log(self.sparsityParam/rho_approx) + (1 - self.sparsityParam)*tf.math.log((1-self.sparsityParam)/(1-rho_approx))) # world model 用的是有截断的klloss
        self.feat_var = tf.reduce_mean(tf.nn.moments(xr, axes=[0])[1])
        self.max_feat = tf.reduce_max(tf.abs(xr))
        self.int_rew = code_b
        self.ae_loss = tf.reduce_mean(tf.square(self.Decoded_obs - self.Original_obs), -1) # 这里没有stop gradient ,两个网络都要训练
        mask = tf.random_uniform(shape=tf.shape(self.ae_loss), minval=0., maxval=1., dtype=tf.float32)
        mask = tf.cast(mask < self.proportion_of_exp_used_for_predictor_update, tf.float32)
        self.aux_loss = tf.reduce_sum(mask * self.ae_loss) / tf.maximum(tf.reduce_sum(mask), 1.) + self.KL_LOSS
