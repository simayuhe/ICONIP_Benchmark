...
from EMP.policies.cnn_gru_policy_dynamics import CnnGruPolicy
from EMP.policies.cnn_policy_param_matched import CnnPolicy
NO_STATES = ['NO_STATES']
...
class InteractionState(object):
    ...
    def __init__(self, ob_space, ac_space, nsteps, gamma, venvs, stochpol, comm):
        ...
        self.buf_vpreds_emp = np.zeros((nenvs, nsteps), np.float32)# EMP
        ...
        self.buf_advs_emp = np.zeros((nenvs, nsteps), np.float32)#EMP
        self.buf_rews_int = np.zeros((nenvs, nsteps), np.float32)
        self.buf_rews_ext = np.zeros((nenvs, nsteps), np.float32)
        self.buf_rews_emp = np.zeros((nenvs, nsteps), np.float32)#EMP
        ...
        self.rff_emp = RewardForwardFilter(gamma)#EMP
        self.rff_rms_emp = RunningMeanStd(comm=comm, use_mpi=True)#EMP
        ...
        self.buf_vpred_emp_last = self.buf_vpreds_emp[:, 0, ...].copy()#EMP
...
class EMPAgent():
    def __init__(self,env,args):
        ...
        self.emp_coeff = args.int_coeff/4 #emp
...
#         print("stochpol_fn^^^^^^^^^^^^^^^^^^^",stochpol_fn)
        ...
        with tf.variable_scope(scope):
            ...
            self.ph_ret_emp = tf.placeholder(tf.float32, [None, None])#emp
            ...
            self.ret_int_rms = RunningMeanStd()#emp
            self.ret_emp_rms = RunningMeanStd()#emp
            self.ret_ext_rms = RunningMeanStd()#emp
            #Define loss.
...
#             vf_loss = vf_loss_int + vf_loss_ext
            ...
            vf_loss = vf_loss_int + vf_loss_ext + vf_loss_emp #EMP
...
#             print("after call , acts",acs)
            ...
            self.I.buf_vpreds_emp[sli, t] = vpreds_emp# EMP
...
#                 _, self.I.buf_vpred_int_last[sli], self.I.buf_vpred_ext_last[sli], _, _, _ = self.stochpol.call(dict_nextobs, nextnews, self.I.mem_state[memsli], update_obs_stats=False)
                _, self.I.buf_vpred_int_last[sli], self.I.buf_vpred_ext_last[sli], self.I.buf_vpred_emp_last[sli], _, _ , _= self.stochpol.call(dict_nextobs, nextnews, self.I.mem_state[memsli], update_obs_stats=False)#EMP
            ...
            fd.update({self.stochpol.ph_mean: self.stochpol.ob_rms.mean,
                       self.stochpol.ph_std: self.stochpol.ob_rms.var ** 0.5})
            fd[self.stochpol.ph_ac] = self.I.buf_acs
            self.I.buf_rews_emp[:] = tf.get_default_session().run(self.stochpol.empowerment_rew, fd) # EMP
