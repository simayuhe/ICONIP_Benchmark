...
from RND.policies.cnn_gru_policy_dynamics import CnnGruPolicy
...
#         print("stochpol_fn^^^^^^^^^^^^^^^^^^^",stochpol_fn)
        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            self.ph_ret_int = tf.compat.v1.placeholder(tf.float32, [None, None])
            self.ph_ret_ext = tf.compat.v1.placeholder(tf.float32, [None, None])
            ...
            self.ph_lr_pred = tf.compat.v1.placeholder(tf.float32, [])
            ...
            clipfrac = tf.reduce_mean(tf.cast(tf.greater(tf.abs(ratio - 1.0), self.ph_cliprange),tf.float32))
            loss = pg_loss + ent_loss + vf_loss + self.stochpol.aux_loss

            #Create optimizer.
            params = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope=self.abs_scope)
            ...
            global_grad_norm = tf.compat.v1.global_norm(grads)
...
#             logger.info(" "*6 + fmt_row(13, self.loss_names))
        ...
        while epoch < self.nepochs:
            ...
            ret = tf.compat.v1.get_default_session().run(self._losses+[self._train], feed_dict=fd)[:-1]
            if not self.testing:
...
#             print("after call , acts",acs)
        ...
        if t == self.nsteps - 1 and not self.disable_policy_update:
            ...
            self.I.buf_rews_int[:] = tf.compat.v1.get_default_session().run(self.stochpol.int_rew, fd)
