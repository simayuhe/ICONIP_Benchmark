...
#                 logger.info("CnnTarget: using '%s' shape %s as image input" % (ph.name, str(ph.shape)))
        ...
        self.int_rew = tf.reduce_mean(tf.square(tf.stop_gradient(X_r) - X_r_hat), axis=-1, keepdims=True)#
        ...
        mask = tf.random.uniform(shape=tf.shape(self.aux_loss), minval=0., maxval=1., dtype=tf.float32)#
